...
int atende_filtros_geral(reg_dados *reg, int m, char nomesCampos[][50], char valoresStr[][100], int valoresInt[])
{
    for (int j = 0; j < m; j++)
    {
        ...
        else if (strcmp(nomesCampos[j], "nomeEstacao") == 0)
        {
            if (valoresStr[j][0] == '\0')
            {
                if (reg->nomeEstacao != NULL && reg->tamNomeEstacao > 0)
                    return 0;
            }
            else
            {
                if (reg->nomeEstacao == NULL)
                    return 0;
                if (strcmp(reg->nomeEstacao, valoresStr[j]) != 0)
                    return 0;
            }
        }
        else if (strcmp(nomesCampos[j], "nomeLinha") == 0)
        {
            if (valoresStr[j][0] == '\0')
            {
                if (reg->nomeLinha != NULL && reg->tamNomeLinha > 0)
                    return 0;
            }
            else
            {
                if (reg->nomeLinha == NULL)
                    return 0;
                if (strcmp(reg->nomeLinha, valoresStr[j]) != 0)
                    return 0;
            }
        }
    }
