...
void funcionalidade11(const char *arq_dados, const char *arq_indice, const char *origem, const char *destino) {
    ...
    // --- 1. Construção do Grafo (compartilhada via grafo_builder) ---
    MapEntry *mapa;
    int tam_map;
    char **nomes_unicos;
    int qtd_nomes;
    ...
    // Identifica os índices numéricos correspondentes à origem e ao destino
    ...
    if (start_idx == -1 || end_idx == -1) {
        ...
        if (dist[end_idx] == INF) {
            ...
            // O array 'path' foi montado do destino para a origem. Vamos imprimir de trás para frente (origem ao destino).
    ...
    // Limpeza de toda a memória alocada dinamicamente e fechamento do arquivo
